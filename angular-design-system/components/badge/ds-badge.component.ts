...
@Component({
  ...
  template: `
    <span
      [ngClass]="[
        'inline-flex items-center font-bold tracking-wider rounded-full uppercase leading-none',
        sizeClasses[size],
        formatVariantClasses[format][variant]
      ]"
    >
      <span
        *ngIf="dot"
...
export class DsBadgeComponent {
  ...
  readonly sizeClasses: Record<DsBadgeSize, string> = {
    sm: 'px-2 py-0.5 text-[10px] gap-1',
    md: 'px-2.5 py-1 text-xs gap-1.5',
  };

  readonly dotColors: Record<DsBadgeVariant, string> = {
    primary: 'bg-[#084c61]',
    accent: 'bg-[#c9a050]',
    contrast: 'bg-[#38563c]',
    dark: 'bg-[#0f2a43]',
  ...
  readonly formatVariantClasses: Record<DsBadgeFormat, Record<DsBadgeVariant, string>> = {
    subtle: {
